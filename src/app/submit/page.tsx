...
import { isPDF, getPdfDimensions, renderPdfThumbnail } from '@/lib/pdf-utils'
...
export default function SubmitJobPage() {
  ...
  const handleFileChange = async (index: number, file: File | null) => {
    ...
    // STRICT: PDFs only. Spot colors must be preserved end-to-end for NeoStampa.
    // Rasterizing to PNG (what the old flow did) breaks custom screen-print ink matching.
    if (!isPDF(file)) {
      setError('PDFs only. Other formats (PNG, JPG, etc.) are not accepted because they cannot preserve the spot colors our printer needs.')
      return
    }

    try {
      // Read vector dimensions directly from the PDF — no rasterization.
      const dims = await getPdfDimensions(file)
      if (dims.page_count > 1) {
        setError(`This PDF has ${dims.page_count} pages. Please upload a single-page PDF (the first page will be used).`)
        // We'll still accept it (use page 1), but warn once so it's visible.
      }
      // Convert inches to the 300 DPI pixel equivalent so sizing-engine math
      // keeps working identically. For vector input, \"px at 300 DPI\" is just
      // a unit; the validation still checks if the target size is sane.
      const widthPxEquiv = Math.round(dims.width_inches * 300)
      const heightPxEquiv = Math.round(dims.height_inches * 300)
      const item = items[index]
      const sizing = calculateTargetSize(widthPxEquiv, heightPxEquiv, 300, item.placement, item.garment_age)
      updateItem(index, {
        file,
        detected_width_px: widthPxEquiv,
        detected_height_px: heightPxEquiv,
...
function ItemForm({ index, item, onUpdate, onFileChange, onPlacementChange, onAgeChange, onRemove, canRemove }: {
  ...
  // Render a PDF thumbnail whenever the item's file changes
  useEffect(() => {
    if (item.file && isPDF(item.file)) {
      let cancelled = false
      renderPdfThumbnail(item.file, 400).then(dataUrl => {
          ...
          {!previewUrl ? (
            ...
            >
              ...
              <div className="pointer-events-none">
                ...
                <p className="text-xs text-gray-600 mt-1">or click to browse — PDF files only</p>
              </div>
            </div>
          ) : (
